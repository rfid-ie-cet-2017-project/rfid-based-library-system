...
void monw(char data[2])
{
    char* lpBuffer = (char*)malloc(2);		       // lpBuffer should be  char or byte array, otherwise write wil fail
    strncpy(lpBuffer, data, 2);
    ...
    char* lBuffer = "\0";
    dNoOFBytestoWrite = sizeof(lBuffer);
    Status = WriteFile(hComm,               // Handle to the Serialport
                       lBuffer,            // Data to be written to the port
                       dNoOFBytestoWrite,   // No of bytes to write into the port
                       &dNoOfBytesWritten,  // No of bytes written to the port
                       NULL);
    if(!Status)
        cout<<"error in writing null"<<endl;
}
